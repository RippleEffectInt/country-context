...
const PLACES=['Kenya','Uganda','Rwanda','Burundi','Ethiopia','Sudan','South Sudan','Somalia','Eritrea','Djibouti','Tanzania'];
...
function hasPlace(s){return PLACES.some(p=>mentions(s,p))}
function splitGeoList(s){
  const first=s.split(/\s*,\s*/).map(x=>x.trim()).filter(Boolean),out=[];
  for(const bit of first){
    const m=bit.match(/^(.+?)\s+and\s+(.+)$/i);
    if(m&&hasPlace(m[1])&&hasPlace(m[2])){out.push(m[1].trim(),m[2].trim())}else out.push(bit.replace(/^and\s+/i,'').trim());
  }
  return out;
}
function cap(s){return s?`${s[0].toUpperCase()}${s.slice(1)}`:s}
function localiseSentence(sentence,name){
  const m=sentence.match(/^(.*?)(?:\s+(?:is|are))?\s+(expected|favoured)\s+over\s+(.+?)[.]?$/i);
  if(!m)return sentence;
  const signal=m[1].trim().replace(/[.:]+$/,'');
  const verb=m[2].toLowerCase();
  const geo=m[3].replace(/[.]+$/,'').trim();
  const fragments=splitGeoList(geo).filter(x=>mentions(x,name));
  if(!fragments.length)return sentence;
  return fragments.map(f=>`${cap(f)} — ${signal.toLowerCase()} ${verb}.`).join(' ');
}
function localiseList(items,name){return items.map(s=>localiseSentence(s,name))}
...
try{
  ...
  for(const[c,name]of EA){
    const rainRaw=pick(ss,name,s=>/rainfall/i.test(s)&&/(more than usual|less than usual|wetter than usual|drier than usual|rainfall anomal)/i.test(s));
    const heavyRaw=pick(ss,name,s=>/90th percentile|exceptional rainfall|heavy rainfall/i.test(s));
    const heatRaw=pick(ss,name,s=>/heat stress|extreme caution|danger category/i.test(s));
    const floodRaw=pick(ss,name,s=>/flood/i.test(s)&&!/mitigate risks associated/i.test(s));
    const tempRaw=pick(ss,name,s=>/temperature/i.test(s)&&/(warmer than usual|cooler than usual|temperature anomal)/i.test(s));
    const rain=localiseList(rainRaw,name),heavy=localiseList(heavyRaw,name),heat=localiseList(heatRaw,name),flood=localiseList(floodRaw,name),temp=localiseList(tempRaw,name);
    out.countries[c].weekly={
      period,
      rainfall:combine(rain,`No specific weekly rainfall anomaly is highlighted for ${name} in the latest ICPAC bulletin.`),
      rainfallScope:rain.length?'country-or-subnational':'none',
      temperature:combine(temp,regionalTemp||`No specific weekly temperature signal is highlighted for ${name} in the latest ICPAC bulletin.`),
      temperatureScope:temp.length?'country-or-subnational':regionalTemp?'regional':'none',
      heavyRain:combine(heavy,`No specific exceptional-rainfall signal is highlighted for ${name} in the latest ICPAC bulletin.`),
      heavyRainScope:heavy.length?'country-or-subnational':'none',
      heatStress:combine(heat,`No specific severe heat-stress signal is highlighted for ${name} in the latest ICPAC bulletin.`),
      heatStressScope:heat.length?'country-or-subnational':'none',
      flood:combine(flood,`No specific flood update is listed for ${name} in the latest ICPAC bulletin. This does not mean there is no local flood risk.`),
      floodScope:flood.length?'country-or-subnational':'none',
      url:WEEKLY
    };
  }
...
try{
  ...
  for(const[c,name]of EA){
    const rainRaw=pick(ss,name,s=>/rainfall/i.test(s)&&/(above normal|below normal|wetter than usual|drier than usual)/i.test(s));
    const tempRaw=pick(ss,name,s=>/temperature/i.test(s)&&/(above normal|below normal|warmer than usual|cooler than usual)/i.test(s));
    const rain=localiseList(rainRaw,name),temp=localiseList(tempRaw,name);
    out.countries[c].seasonal={
      period,
      rainfall:combine(rain,`No specific seasonal rainfall signal is highlighted for ${name} in the latest ICPAC summary.`),
      rainfallScope:rain.length?'country-or-subnational':'none',
      temperature:combine(temp,regionalTemp||'See the latest ICPAC seasonal temperature outlook.'),
      temperatureScope:temp.length?'country-or-subnational':regionalTemp?'regional':'none',
      url:SEASONAL
    };
  }
...
try{
  ...
  out.countries.ZMB.weekly={period:'Current',rainfall:"ICPAC's weekly forecast does not cover Zambia. Shorter-range information should come from Zambia Meteorological Department or SADC services.",rainfallScope:'not-covered',temperature:"ICPAC's weekly forecast does not cover Zambia.",temperatureScope:'not-covered',heavyRain:'No ICPAC weekly product is applied to Zambia.',heavyRainScope:'not-covered',heatStress:'No ICPAC weekly product is applied to Zambia.',heatStressScope:'not-covered',flood:'No ICPAC weekly flood product is applied to Zambia.',floodScope:'not-covered',url:ZMD};
  out.countries.ZMB.seasonal={period:'2026/27 rainy season',rainfall:combine(rain,'See the latest SADC/SARCOF seasonal rainfall outlook for Zambia.'),rainfallScope:'country-or-subnational',temperature:temp||'See the latest SADC/SARCOF temperature outlook.',temperatureScope:'regional',url:SADC};
