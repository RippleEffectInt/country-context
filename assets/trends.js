...
function direction(points,format){const p=(points||[]).filter(x=>Number.isFinite(Number(x.value)));if(p.length<2)return'';const a=Number(p.at(-2).value),b=Number(p.at(-1).value),diff=b-a;if(Math.abs(diff)<1e-9)return`<span class="trend-direction">No change since previous source period</span>`;const amount=String(format(Math.abs(diff))).replace(/^\+/,'');return `<span class="trend-direction">${diff>0?'↑':'↓'} ${esc(amount)} since previous source period</span>`}
const defs={
